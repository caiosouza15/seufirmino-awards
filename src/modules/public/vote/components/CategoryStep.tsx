  ...
  return (
    <div className="mx-auto max-w-xl space-y-6 overflow-hidden rounded-3xl border border-violet-500/40 bg-slate-900/70 p-6 shadow-[0_0_18px_rgba(139,92,246,0.25)] md:p-8">
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-cyan-200/80">Quarteto Awards</p>
        <h2 className="text-2xl font-bold text-white md:text-3xl">{category.name}</h2>
        {category.description && <p className="text-sm text-slate-300">{category.description}</p>}
      </div>
      ...
      <div className="space-y-3">
        <p className="text-sm text-slate-300">Escolha 1 opcao para continuar.</p>
        <NeonButton
          fullWidth
          variant="primary"
          size="lg"
          ...
          onClick={onConfirm}
        >
          {isLastCategory ? "Confirmar e finalizar votacao" : "Confirmar e ir para a proxima"}
        </NeonButton>
      </div>
    </div>
  );
